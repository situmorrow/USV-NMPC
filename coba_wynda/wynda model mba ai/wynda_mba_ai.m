...
% Kecepatan maju/surge rata-rata (U) dari data eksperimen GPS (speedMps)
U = 0.5923;      % default fallback
csv_candidates = {'DATA_FIKS.csv', 'ZIGZAG.csv', 'lurus_konstan.csv'};
for idx = 1:length(csv_candidates)
    if exist(csv_candidates{idx}, 'file')
        t_exp = readtable(csv_candidates{idx});
        if ismember('speedMps', t_exp.Properties.VariableNames)
            speed_valid = t_exp.speedMps(t_exp.speedMps > 0 & ~isnan(t_exp.speedMps));
            if ~isempty(speed_valid)
                U = mean(speed_valid);
                fprintf('Kecepatan surge rata-rata U diperoleh dari %s: %.4f m/s\n', csv_candidates{idx}, U);
                break;
            end
        end
    end
...
for ep = 1:num_epochs
    ...
    for i = 1:N
        ...
        % ===== BASIS FUNCTION (Persamaan 4.27 - 4.31) =====
        Phi = [
