...
int computeConstraints_(const ::coder::internal::i_stickyStruct &obj,
                        const double x[20], double Cineq_workspace[80])
{
  double varargout_1[40];
  int idx_current;
  int status;
  bool allFinite;
  nmpc_kapal_waypoint_anonFcn2(
      obj.next.next.next.next.next.next.next.value.workspace.current_state_nd,
      obj.next.next.next.next.next.next.next.value.workspace.r_limit_nd, x,
      varargout_1);
  std::copy(&varargout_1[0], &varargout_1[40], &Cineq_workspace[40]);
  status = 1;
  allFinite = true;
  idx_current = 40;
  while (allFinite && (idx_current + 1 <= 80)) {
