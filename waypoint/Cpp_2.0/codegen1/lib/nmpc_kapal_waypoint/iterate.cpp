...
void iterate(const double H[400], const double f[101], f_struct_T &solution,
             ...
             int runTimeOptions_MaxIterations)
{
  ...
  std::memset(&solution.lambda[0], 0, 201U * sizeof(double));
  int exitg1;
  do {
    exitg1 = 0;
    if (solution.state == -5) {
      ...
      if (subProblemChanged) {
        ...
        case 1:
          QRManager::squareQ_appendCol(qrmanager, workingset.ATwset,
                                       101 * (workingset.nActiveConstr - 1) +
                                           1);
          break;
        case -1:
        ...
        if (solution.state != -5) {
          ...
          if (updateFval) {
            ...
            case 3:
              ...
              ret = 101 * (idx_local - 1);
              iAw0 = 101 * (workingset.nActiveConstr - 1);
              i = workingset.nVar;
      ...
      if (guard1) {
        ...
        if (solution.iterations - div_nzp_s32(solution.iterations) * 50 == 0) {
          ...
          if (minLambda > 1.0E-6) {
            if (ret - 1 >= 0) {
              std::copy(&solution.xstar[0], &solution.xstar[ret],
                        &solution.searchDir[0]);
            }
            updateFval = initialize::feasibleX0ForWorkingSet(
            ...
            activeSetChangeID = 0;
            minLambda = WorkingSet::b_maxConstraintViolation(
                workingset, solution.searchDir);
            if (minLambda < solution.maxConstr) {
              if (ret - 1 >= 0) {
                std::copy(&solution.searchDir[0], &solution.searchDir[ret],
                          &solution.xstar[0]);
              }
