...
void partialColLDL3_(i_struct_T &obj, int LD_offset, int NColsRemain)
{
  ...
  for (int k{0}; k < 48; k++) {
    ...
    LD_diagOffset = (LD_offset + 202 * k) - 1;
  ...
  for (int j{48}; j <= i; j += 48) {
    ...
    for (int k{j}; k < LD_diagOffset; k++) {
      ...
      for (int idx{0}; idx < 48; idx++) {
        d = obj.FMat[((LD_offset + k) + idx * 201) - 1];
      }
      obj.workspace2_ = d;
      ix = k + 1;
      if (m != 0) {
        br = k + 9448;
        for (int idx{ix}; idx <= br; idx += 201) {
          i1 = idx + m;
          for (int ia{idx}; ia < i1; ia++) {
    ...
    if (LD_diagOffset < NColsRemain) {
      ...
      ic0 = ((LD_offset + subBlockSize) + 202 * j) - 1;
      for (int idx{0}; idx < 48; idx++) {
        ix = (LD_offset + j) + idx * 201;
        for (int ia{0}; ia < subBlockSize; ia++) {
          obj.workspace2_ = obj.FMat[(ix + ia) - 1];
        }
      }
      if ((m != 0) && (subBlockSize != 0)) {
        ix = ic0 + 201 * (subBlockSize - 1);
        br = 0;
        for (int idx{ic0}; idx <= ix; idx += 201) {
          br++;
          i1 = br + 9447;
          for (int ia{br}; ia <= i1; ia += 201) {
            subRows = idx + 1;
            LD_diagOffset = idx + m;
