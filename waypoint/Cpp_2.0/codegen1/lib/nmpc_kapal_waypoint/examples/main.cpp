...
void main_nmpc_kapal_waypoint()
{
  ...
  u_prev_tmp = argInit_real_T();
  // Initialize function input argument 'x_ref_seq'.
  argInit_20x1_real_T(x_ref_seq_tmp);
  ...
  argInit_5x1_real_T(b_dv);
  nmpc_kapal_waypoint(b_dv, u_prev_tmp, u_prev_tmp, x_ref_seq_tmp,
                      x_ref_seq_tmp, x_ref_seq_tmp, &u_opt, &exitflag);
}
