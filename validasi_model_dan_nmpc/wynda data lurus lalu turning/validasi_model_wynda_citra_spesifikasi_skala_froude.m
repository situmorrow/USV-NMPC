...
% =============================================================================
% * Validasi open-loop model non-linear WyNDA berbasis persamaan hidrodinamika
%   hasil estimasi parameter adaptive observer (Theta 1 s.d. Theta 22).
...
%% =========================================================================
%  2. SPESIFIKASI KAPAL & PARAMETER THETA WYNDA
% =========================================================================
...
theta = [
   -2.3170e-01;  % theta_1  : v'
    6.1177e-02;  % theta_2  : r'
    1.3593e+00;  % theta_3  : (v')^3
   -2.0967e+00;  % theta_4  : (v')^2 * r'
    1.3877e+00;  % theta_5  : v' * (r')^2
   -3.0868e-01;  % theta_6  : (r')^3
    3.7619e-03;  % theta_7  : sec(delta)
    6.1177e-02;  % theta_8  : u0' * r'
   -1.4318e-01;  % theta_9  : v' pada r_dot
    3.0723e-02;  % theta_10 : r' pada r_dot
    9.9806e-01;  % theta_11 : (v')^3 pada r_dot
   -1.6924e+00;  % theta_12 : (v')^2 * r' pada r_dot
    1.0730e+00;  % theta_13 : v' * (r')^2 pada r_dot
   -2.0955e-01;  % theta_14 : (r')^3 pada r_dot
    1.9423e-03;  % theta_15 : n^2 (rpm^2)
   -2.1837e-02;  % theta_16 : sin(delta)*sec^2(delta)
   -2.3049e-03;  % theta_17 : sec(delta) pada r_dot
   -6.0670e-02;  % theta_18 : u0' * cos(psi')
   -9.3868e-02;  % theta_19 : v' * sin(psi')
   -5.7507e-02;  % theta_20 : u0' * sin(psi')
    3.2559e-02;  % theta_21 : v' * cos(psi')
    5.9990e-02   % theta_22 : r' pada psi_dot
];
...
%% =========================================================================
%  6. FUNGSI BASIS PHI MODEL WYNDA (Persamaan 4.13 - 4.17)
% =========================================================================
...
function Phi = compute_basis_phi(s, delta, rpm_nd, u0_nd)
    ...
    % Persamaan (4.13) -> Row 1 (f1: delta_v')
    Phi(1, 1) = v;
    Phi(1, 2) = r;
    Phi(1, 3) = v^3;
    Phi(1, 4) = (v^2) * r;
    ...
    % Persamaan (4.14) -> Row 2 (f2: delta_r')
    Phi(2, 9)  = v;
    Phi(2, 10) = r;
    Phi(2, 11) = v^3;
    Phi(2, 12) = (v^2) * r;
    ...
    Phi(2, 14) = r^3;
    Phi(2, 15) = rpm_nd^2;
    Phi(2, 16) = sin(delta) * (sec(delta)^2);
    Phi(2, 17) = sec(delta);
    
    % Persamaan (4.15) -> Row 3 (f3: delta_x')
    Phi(3, 18) = u0_nd * cos(psi);
    Phi(3, 19) = v * sin(psi);
    
    % Persamaan (4.16) -> Row 4 (f4: delta_y')
    Phi(4, 20) = u0_nd * sin(psi);
    Phi(4, 21) = v * cos(psi);
    
    % Persamaan (4.17) -> Row 5 (f5: delta_psi')
    Phi(5, 22) = r;
end
