                    ...
                    bool alwaysPositiveDef)
{
  ...
  if (mNull <= 0) {
    ...
    if (qrmanager.ncols <= 0) {
      ...
      case 3: {
        ...
        if (alwaysPositiveDef) {
          ...
          if (cholmanager.ConvexCheck) {
            ...
            do {
              exitg2 = 0;
              if (jA <= ix - 1) {
                if (cholmanager.FMat[jA + 201 * jA] <= 0.0) {
        ...
        if (cholmanager.info != 0) {
          ...
          for (int idx{0}; idx < jjA; idx++) {
            solution.searchDir[idx] /= cholmanager.FMat[idx + 201 * idx];
          }
          if (cholmanager.ndims != 0) {
      ...
      int nullStartIdx;
      nullStartIdx = 201 * qrmanager.ncols + 1;
      if (objective.objtype == 5) {
        for (int idx{0}; idx < mNull; idx++) {
          memspace.workspace_float[idx] =
              -qrmanager.Q[nVar + 201 * (qrmanager.ncols + idx)];
        }
        if (qrmanager.mrows != 0) {
          ...
          ix = 0;
          k = nullStartIdx + 201 * (mNull - 1);
          for (int idx{nullStartIdx}; idx <= k; idx += 201) {
            int jA;
            jA = idx + nVar;
        ...
        if (objective.objtype == 3) {
          ...
          for (int idx{0}; idx < mNull; idx++) {
            for (int b_k{jA}; b_k <= k; b_k++) {
              memspace.workspace_float[(b_k + 201 * idx) - 1] =
                  objective.beta *
                  qrmanager.Q[(b_k + 201 * (idx + qrmanager.ncols)) - 1];
            }
          }
          internal::blas::xgemm(mNull, mNull, qrmanager.mrows, qrmanager.Q,
                                nullStartIdx, memspace.workspace_float,
        ...
        if (cholmanager.info != 0) {
          ...
          if (qrmanager.mrows != 0) {
            ...
            jA = nullStartIdx + 201 * (mNull - 1);
            for (int b_k{nullStartIdx}; b_k <= jA; b_k += 201) {
              temp = 0.0;
              k = b_k + nVar;
              for (int idx{b_k}; idx <= k; idx++) {
                temp += qrmanager.Q[idx - 1] * objective.grad[idx - b_k];
              }
              k = div_nzp_s32_floor(b_k - nullStartIdx, 201);
