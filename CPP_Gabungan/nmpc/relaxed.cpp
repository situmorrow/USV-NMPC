...
namespace step {
void b_relaxed(const double Hessian[400], const double grad[101],
               ...
               struct_T &QPObjective, k_struct_T &qpoptions)
{
  ...
  for (int iac{0}; iac <= 7979; iac += 101) {
    ...
    temp = div_nzp_s32_floor(iac, 101);
  ...
  for (int idx{0}; idx < 80; idx++) {
    ...
    tf = WorkingSet.isActiveConstr
             [((WorkingSet.isActiveIdx[3] + WorkingSet.sizes[3]) + idx) - 81];
  ...
  if (b_TrialState.state != -6) {
    ...
    c = 0.0;
    if (100 - nVarOrig >= 1) {
      for (int idx{temp}; idx < 101; idx++) {
        c += std::abs(b_TrialState.xstar[idx - 1]);
      }
    }
    qpfvalQuadExcess = 0.0;
    if (100 - nVarOrig >= 1) {
      temp = static_cast<unsigned char>(100 - nVarOrig);
      for (int idx{0}; idx < temp; idx++) {
