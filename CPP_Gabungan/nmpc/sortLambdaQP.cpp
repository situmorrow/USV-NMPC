...
void sortLambdaQP(double lambda[201], int WorkingSet_nActiveConstr,
                  const int WorkingSet_sizes[5],
                  const int WorkingSet_isActiveIdx[6],
                  const int WorkingSet_Wid[201],
                  const int WorkingSet_Wlocalidx[201], double workspace[20301])
{
  if (WorkingSet_nActiveConstr != 0) {
    int currentMplier;
    int idx;
    int mAll;
    mAll = WorkingSet_sizes[3] + 99;
    currentMplier = static_cast<unsigned char>(WorkingSet_sizes[3] + 100);
