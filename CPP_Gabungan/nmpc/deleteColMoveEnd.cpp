...
void deleteColMoveEnd(c_struct_T &obj, int idx)
{
  ...
  if (idx >= obj.ncols) {
    ...
    i = obj.minRowCol;
    for (int k{0}; k < i; k++) {
      obj.QR[k + 201 * (idx - 1)] = obj.QR[k + 201 * (obj.ncols - 1)];
    }
    obj.ncols--;
    i = obj.mrows;
    u1 = obj.ncols;
    ...
    if (idx < obj.mrows) {
      ...
      while (b_k >= idx) {
        ...
        for (int k{0}; k < i; k++) {
          u1 = b_i + k;
          temp = c * obj.Q[u1] + s * obj.Q[u1 + 201];
          obj.Q[u1 + 201] = c * obj.Q[u1 + 201] - s * obj.Q[u1];
          obj.Q[u1] = temp;
        }
        b_k--;
      }
