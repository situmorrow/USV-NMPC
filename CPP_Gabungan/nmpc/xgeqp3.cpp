...
namespace internal {
namespace lapack {
void xgeqp3(double A[40401], int m, int n, int jpvt[201], double tau[201])
{
  double vn1[201];
  double vn2[201];
  double work[201];
  ...
  std::memset(&tau[0], 0, 201U * sizeof(double));
  if (minmn < 1) {
    for (int j{0}; j < n; j++) {
      jpvt[j] = j + 1;
    }
    ...
    if (nfxd < minmn) {
      ...
      std::memset(&vn1[0], 0, 201U * sizeof(double));
      std::memset(&vn2[0], 0, 201U * sizeof(double));
      ix = nfxd + 1;
      for (int j{ix}; j <= n; j++) {
        temp = blas::xnrm2(m - nfxd, A, (nfxd + (j - 1) * 201) + 1);
        vn1[j - 1] = temp;
        vn2[j - 1] = temp;
      }
      i = nfxd + 1;
      for (int k{i}; k <= minmn; k++) {
        ...
        if (k < n) {
          temp = A[ii];
          A[ii] = 1.0;
          reflapack::xzlarf(mmi + 1, nmi - 1, ii + 1, temp1, A, ii + 202, work);
          A[ii] = temp;
        }
        for (int j{ip1}; j <= n; j++) {
          ix = k + (j - 1) * 201;
