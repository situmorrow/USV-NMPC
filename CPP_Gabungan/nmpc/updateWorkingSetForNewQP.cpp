...
namespace internal {
void updateWorkingSetForNewQP(const double xk[20], g_struct_T &WorkingSet,
                              const double cIneq[80], int mLB)
{
  int i;
  ...
  for (int idx{0}; idx < i; idx++) {
    WorkingSet.lb[WorkingSet.indexLB[idx] - 1] =
        xk[WorkingSet.indexLB[idx] - 1] + 0.78539816339744828;
  }
  for (int idx{0}; idx < 20; idx++) {
    WorkingSet.ub[WorkingSet.indexUB[idx] - 1] =
        0.78539816339744828 - xk[WorkingSet.indexUB[idx] - 1];
  }
  if (WorkingSet.nActiveConstr > 0) {
    ...
    for (int idx{1}; idx <= i; idx++) {
      ...
      default: {
        ...
        if (WorkingSet.Wlocalidx[idx - 1] > 40) {
          int i1;
          int ix0;
          int iy0;
          iy0 = 101 * (idx - 1);
          ix0 = 101 * (WorkingSet.Wlocalidx[idx - 1] - 1);
